...
def main():
    ...
    global disc_actions
    # env = SubprocVecEnv([
    #     wrap_cover_pendulun(ENV_NAME, disc_actions, render_mode="human", episode_length=EPISODE_LENGTH) for i in range(N_ENVS)])
    # env = SubprocVecEnv([
    #     wrap_cover_pendulun_test(ENV_NAME, disc_actions, render_mode="human", episode_length=EPISODE_LENGTH) for i in
    #     range(N_ENVS)])
    env = SubprocVecEnv([
        wrap_cover_pendulun(ENV_NAME, disc_actions, episode_length=EPISODE_LENGTH) for i in
        range(N_ENVS)])
    ...
    for step in range(1, STEP_NUM // N_ENVS + 1):
        ...
        if step % SAVE_FREQ == 0:
            ...
            if mean_100_ep_return >= -200:
                # show the animation and record the mean_100_ep_return (the average return over the last 100 episodes)
